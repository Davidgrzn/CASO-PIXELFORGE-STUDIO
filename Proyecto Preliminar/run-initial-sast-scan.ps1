...
$InitialReportDir = Join-Path $ReportsDir "initial-scan"
$FinalReportDir = Join-Path $ReportsDir "final-scan"

New-Item -ItemType Directory -Force -Path $InitialReportDir, $FinalReportDir | Out-Null
...
function Wait-SonarQube {
    ...
    Write-Host "Esperando a que SonarQube esté disponible..."
...
1. Genera un token en SonarQube: http://localhost:9000 > My Account > Security > Generate Tokens
   ...
   .\run-initial-sast-scan.ps1

2. O pasa credenciales explícitas:
   .\run-initial-sast-scan.ps1 -SonarLogin admin -SonarPassword "TU_PASSWORD"
"@
    }
}
...
Write-Host "=========================================="
Write-Host "PixelForge Studio - Análisis SAST INICIAL"
Write-Host "=========================================="
Write-Host "Directorio: $ProjectDir"

Write-Host ""
Write-Host "[1/3] Iniciando SonarQube en Docker..."
...
Write-Host ""
Write-Host "[2/3] Ejecutando SonarQube Scanner..."
Ensure-SonarScanner
...
Write-Host ""
Write-Host "[3/3] Ejecutando Bearer CLI..."
Run-BearerScan -WorkDir $BackendDir -ScanPath "app" -OutputPath (Join-Path $InitialReportDir "bearer-backend-initial.html")
Run-BearerScan -WorkDir $FrontendDir -ScanPath "src" -OutputPath (Join-Path $InitialReportDir "bearer-frontend-initial.html")

Write-Host ""
Write-Host "=========================================="
Write-Host "Análisis SAST inicial terminado"
Write-Host "SonarQube: $SonarHost"
Write-Host "Reportes: $InitialReportDir"
Write-Host "=========================================="
